...
def save_trade(coin, symbol, action, entry, target, sl):
    if os.path.exists(TRADE_FILE):
        ...
        if not df[(df['Coin'] == coin) & (df['Status'].str.contains('Active|Trailing'))].empty: return 
    else:
...
if app_mode == "🔍 Crypto Auto-Scanner":
    st.title("🪙 Crypto AI Scanner (Multi-Timeframe)")
            ...
            if "Active" in row['Status'] or "Trailing" in row['Status']:
                try:
                    ...
                    if "BUY" in row['Action']:
                        ...
                        if curr_price >= target: df.at[index, 'Status'] = "🏆 Target Hit"
                        elif curr_price <= float(row['SL']): df.at[index, 'Status'] = "💔 SL Hit"
