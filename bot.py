...
st_autorefresh(interval=60000, limit=1000, key="india_refresh") # 60 seconds auto-refresh
...
# यहाँ से बटन हटा दिया है, अब यह अपने आप स्कैन करेगा
with st.spinner("Market Trend & Smart Money Scanning..."):
    ...
    for name, ticker_symbol in stock_list.items():
        try:
            ...
            if not data.empty:
                ...
                if bullish:
                    results.append({"Index/Stock": name, "Action": "🟢 BUY (CE)", "Entry": f"₹{curr:.2f}", "Target": f"₹{curr + tgt_val:.2f}", "SL": f"₹{curr - sl_val:.2f}"})
                    send_telegram_alert(f"🚀 INDIAN MARKET BUY: {name}\nEntry: ₹{curr:.2f}\nTarget: ₹{curr + tgt_pts:.2f}\nSL: ₹{curr - sl_pts:.2f}")
                elif bearish:
                    results.append({"Index/Stock": name, "Action": "🔴 SELL (PE)", "Entry": f"₹{curr:.2f}", "Target": f"₹{curr - tgt_val:.2f}", "SL": f"₹{curr + sl_val:.2f}"})
    ...
    if results:
        ...
        st.warning("⚖️ Market Side-ways hai. AI wait kar रहा है परफेक्ट ब्रेकआउट का...")
